...
export function WhyChooseSection() {
  return (
    <section id="diferenciais" className="relative py-28 sm:py-36">
      ...
      <Container className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <article className="h-full rounded-[2rem] border border-brand-parchment/[0.08] bg-white/[0.02] p-9 md:p-12">
            ...
            <ul className="mt-12 grid gap-5">
              {reasons.map((reason) => (
                <li
                  key={reason}
                  className="group flex items-start gap-4 rounded-2xl border border-brand-parchment/[0.08] bg-black/15 p-6 transition-all duration-400 hover:border-brand-champagne/20 hover:bg-black/25"
                >
                  <CheckCircle2
                    size={20}
                    strokeWidth={1.5}
                    className="mt-0.5 shrink-0 text-brand-champagne/60 transition-colors duration-300 group-hover:text-brand-champagne"
                  />
                  <span className="text-[15px] leading-[1.8] text-brand-parchment/60">{reason}</span>
                </li>
              ))}
            </ul>
          </article>
        ...
        <Reveal delay={0.12}>
          <article className="flex h-full flex-col justify-between rounded-[2rem] border border-brand-champagne/20 bg-gradient-to-br from-brand-walnut/70 via-brand-charcoal to-brand-ebony p-9 md:p-12">
            <div>
            ...
            <div className="mt-10">
              <div className="rounded-2xl border border-brand-parchment/10 bg-black/20 p-6 backdrop-blur-sm">
                <p className="text-[15px] leading-relaxed text-brand-parchment/55">
                ...
                <Link
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="btn-shimmer mt-5 inline-flex rounded-full bg-brand-champagne px-6 py-3 text-sm font-semibold text-brand-ebony transition-all duration-300 hover:shadow-glow"
                >
                  Solicitar orçamento
                </Link>
              </div>
